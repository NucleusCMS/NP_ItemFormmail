...
class NP_ItemFormmail extends NucleusPlugin {
	...
	function getMinNucleusVersion() { return 230; }
	...
	function install() {
		...
		$this->createOption("autoreplyfrom",
		_IFORM_OPTION_AUTOREPLY_FROM, "text", $CONF['AdminEmail']);
		$this->createOption("autoreplyfromjp",
		_IFORM_OPTION_AUTOREPLY_FROMJP, "text", $CONF['SiteName']);
		$this->createOption("autoreplysubject",
	...
	*/
	 ...
	 */
	function parse_uform($idx, $type, $colname, $jpname = '' ,$option = '' ,$appendix = '') {
		if ($idx == "option") {
			$this->options[] = array ($type => $colname);
		} else {
			$postvarname = $this->postvarname;
			echo $this->_gene_formElement($idx, $type, $colname, $jpname ,$option  ,$appendix );
		}
	}
	/**
	 * Using NP_Captcha
	...
	*/
	...
	// interpret post data
	//
	function receipt_post($itemid = 0) {
		global $CONF;
		$postvarname = $this->postvarname;
		...
		switch ( $this->mode ) {
			case 'send':
				// title
				$this->receipt['contents'] = $this->form['formname']."\n\n";
				// mail check
				...
				$this->receipt['contents'] .=  "\n\n\n-----access infomation-----\n\n";
				$this->receipt['contents'] .=  "Access time = ".date("ymd:h.i.s",time())."\n";
				$this->receipt['contents'] .=  "Remote address = ".$_SERVER['REMOTE_ADDR']."\n";
				$this->receipt['contents'] .=  "Send from = ".$backURL."\n";
				$this->receipt['contents'] .=  "Browser = ".$_SERVER['HTTP_USER_AGENT']."\n\n";
				// set custom subject
	...
	function _parse_preview() {
		...
		// start contents data form
			...
			// flags
			$previewedform .= '<input name="previewed" type="hidden" id="subject" value="true"/>'.$linefeed;
			$previewedform .= '<input name="sendmail" type="hidden" id="subject" value="true"/>'.$linefeed;
			$previewedform .= "</form>\n";
		...
		// start rewrite form
			...
			$rewriteform .= '<input name="rewrite" type="hidden" id="subject" value="true" />'.$linefeed;
			$rewriteform .= '<input name="sendmail" type="hidden" id="subject" value="true"/>'.$linefeed;
...
/* edit end yamamoto*/
	...
	*/
	function _gene_formElement($idx, $type, $colname, $jpname = '' ,$option = '' ,$appendix = '' ) {
		$postvarname = $this->postvarname;
		if (isset($_POST[$postvarname][$idx]['value'])) {
			$columnValue = $_POST[$postvarname][$idx]['value'];
		}
		$columnValue = htmlspecialchars($columnValue);
		switch ($type) {
			default:
			case "text":
			case "textfield":
		...
		$text .= "<input name='".$postvarname."[$idx][id]' type='hidden' value='$colname'/>\n";
		return $text;
	}
	//
	// result messages
